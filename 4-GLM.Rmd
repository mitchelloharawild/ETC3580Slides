...
author: "Week 4: Generalized Linear Models"
fontsize: 14pt
output:
  beamer_presentation:
    theme: metropolis
 ...
 * Makes estimation easy and inference exact.
...
\begin{block}{Robust Maximum likelihood estimates:}
\centerline{$\displaystyle
\sum_{i=1}^n \gamma_i \left[\frac{ y_i -b'(\theta_i) }{V(\mu_i)}\right]
 \frac{\partial \mu_i}{\partial \beta_j}  = 0\qquad \text{for all $j$}
$}
where $\gamma_i$ downweights extreme observations.
\end{block}

* $\gamma$ is estimated iteratively.
* Implemented in R using `robustbase::glmrob` for binomial and Poisson.
